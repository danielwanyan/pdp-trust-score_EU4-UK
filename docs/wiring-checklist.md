...
This checklist is for the merged UK+EU4 vNext Aicolate workflow. It evaluates a single product per invocation and one product per workflow call. The active workflow has exactly one LLM and only one LLM: `Trust_Evaluator`.
...
`target_country` must be one of `UK`, `DE`, `FR`, `ES`, or `IT`. Apply strict target-country language rules for all locale-facing evidence. `is_new_product_30d` must be passed through to `new_product_context`.
...
Declare these Output Panel fields:
...
`product_main_images` comes from `ProductMeta.images`. `size_chart_images` comes from `product_extra_attributes.size_chart`. `product_attributes_text` comes from `product_extra_attributes.product_attributes`. `images` is the combined visual list for LLM image input, while `image_manifest` keeps source counts visible.
...
`Trust_Evaluator` is the only LLM node. Use flat input names and `{{variable_name}}` references in prompts.
...
## Decision Rules To Confirm In Prompt

- Strict target-country language applies to title, product description, `product_main_images`, `size_chart_images`, and `product_attributes_text`; missing target-country primary language for core purchase facts can cap `page_quality <= 3`.
- `visual_evidence_context` must keep `product_main_images` and `size_chart_images` separated while `images` can carry the combined image list.
- `price_input_semantics` must be followed exactly: UK GBP native inputs, EU4 USD-to-EUR conversion, and `list_price_usd` anchor only.
- `is_new_product_30d` and `new_product_context` reduce product-review-history weight only when the product is a confirmed 30-day-new product; this cannot override safety, compliance, IPR, visible mismatch, or negative review evidence.
- Score 3 is the deboost tier and needs concrete evidence. Products with clean evidence should not be mechanically pushed to score 3 because they are new or sparse.
