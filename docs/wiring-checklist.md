...
Use this checklist for the isolated `pdp-trust-score_EU4-UK` merged vNext workflow.
...
Start / Excel row
  ...
  ├─ HTTP Request: rules_text_fetch
  └─ HTTP Request: rules_json_fetch
...
The active workflow has exactly one LLM node: `Trust_Evaluator`. It is a single product per invocation and one product per workflow call. There is only one LLM in the production path.
...
`product_image_rpc` fetches high-resolution product images:

```text
Feature code: ProductMeta.images
```

Map it into `Final_data_cleaning`:

```text
rpc_images = product_image_rpc.output
```

`product_extra_attributes_rpc` fetches product attributes and size chart evidence:
...
Map structured rules into `Rules_Brain`:
...
Map all Start fields above, plus:
...
image_source = Final_data_cleaning.image_source
image_manifest = Final_data_cleaning.image_manifest
product_attributes_text = Final_data_cleaning.product_attributes_text
product_desc = Final_data_cleaning.product_desc
target_country = Final_data_cleaning.target_country
...
- If core information is not localized into the target-country primary language, `page_quality <= 3`.
